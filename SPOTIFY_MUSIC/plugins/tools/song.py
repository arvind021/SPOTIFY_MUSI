...
async def d(c,q:CallbackQuery):
    ...
    if os.path.exists(p)and os.path.getsize(p)>0:return await f(c,cid,p,v)
    try:
        async with aiohttp.ClientSession() as s:
            ...
            else:return await c.send_message(cid,"🚫 Server Busy, Try Again Later")
        proc=await asyncio.create_subprocess_shell(f'curl -L "{u}" -o "{p}"');await proc.communicate()
        if not os.path.exists(p)or os.path.getsize(p)==0:return await c.send_message(cid,"❌ Download failed")
    except Exception as e:return await c.send_message(cid,f"❌ {e}")
    await f(c,cid,p,v)
